...
// ------------------------------------------------------------------------
//LXCMS自定义函数
function Getimg($aid,$imgwith=210,$imgheight=164,$num=4) {    
    global $dsql;    
    $imgurls = '';    
...
function Getimgs($aid, $imgwith = 210, $imgheight = 164, $num = 4, $style = ''){
    global $dsql;
    $imgurls = '';
     $row = $dsql -> getone("Select imgurls From`#@__addonimages` where aid='$aid'"); //
     $imgurls = $row['imgurls'];
    ...
    for($i = 0;$i < $num;$i++){
        $str1 = substr($wordcount[1][$i],15);
        $str2 = explode("' ",$str1);
        $str3 = $str2[0];
        if($style == 'li'){
            $imglist .= "<li><ahref=#" . $i . "><imgsrc=". trim($wordcount[2][$i]) . " width=" . $imgwith . " height=" . $imgheight . "></li></a><span><a href='[field:arcurl /]'>[field:fulltitle/]</a></span>";
        }else{
            $imglist .= "<li><a title=" . $str3 . " href=" . trim($wordcount[2][$i]) . "><img src= " . trim($wordcount[2][$i]) . " width='" . $imgwith . "' height='" . $imgheight . "'></a><span><a title=" . $str3 . "  href=" . trim($wordcount[2][$i]) . ">".$str3."</a></span></li>";
        }
    }
    return $imglist;
}
